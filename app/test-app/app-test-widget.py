...
class MyOptionMenu(customtkinter.CTkFrame):
    def __init__(self, master, values=["option 1", "option 2"], label_text="Options:"):
        ...
        ## Label
        self.optionmenu_label = customtkinter.CTkLabel(
            self,
            text=label_text,
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.my_optionmenu.grid(
            row=0, column=0, padx=10, pady=(10, 0), sticky="w", columnspan=2
        )
        self.my_optionmenu.configure(fg_color="transparent")
        # My Entry
        self.my_entry = MyEntry(self, placeholder_text="Hello")
        self.my_entry.grid(
            row=1, column=0, padx=10, pady=(10, 0), sticky="w", columnspan=2
        )
        self.my_entry.configure(fg_color="transparent")

        # Button
        self.button = customtkinter.CTkButton(
            master=self, text="CTkButton", command=self.button_event
        )
        self.button.grid(
            row=2, column=0, padx=5, pady=(10, 0), sticky="ew", columnspan=2
        )
